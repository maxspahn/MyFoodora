...
different parts which can be treated separately. This logic separation of different parts of the
...
\begin{itemize}
	...
	\item{\textbf{user-management}} Manages users of the systems and assesses functions
		to groups of users.
...
\textit{MyFoodora}. The separation chosen allowed us to separately work on the
...
\textsc{ItemFactoryProducer} to produce new items and store them in an Array-List. Following you
...
existing code. Another advantage of this design that the programmer do not even has to add a new
...
argument for each type instead of an \textit{ArrayList} with all single-Items. Because of that
